...
from typing import TYPE_CHECKING, Any, Dict, List, Union, assert_never
...
def encode_image_to_base64(image: np.ndarray | str) -> np.ndarray | str | None:
    if isinstance(image, np.ndarray):
        image = Image.fromarray(image.astype("uint8"))  # ty:ignore[invalid-assignment, no-matching-overload]
    elif isinstance(image, str):
        if image.startswith("http://") or image.startswith("https://"):
            image = load_image_from_url(image)  # ty:ignore[invalid-assignment]
...
def serialize_image(image: np.ndarray | torch.Tensor | Image.Image):
    if isinstance(image, np.ndarray):
        shape = image.shape
        dtype = image.dtype
    ...
    if isinstance(image_str, dict) and image_str.get("cls", None):
        ...
        if cls == "ndarray":
            ...
            if image_str.get("dtype", None) is None:
                log.warning("Missing dtype for ndarray")
            ...
            if image_str.get("device", None) is None:
                log.warning("Missing device for Tensor")
            shape = tuple(image_str["shape"])
            dtype = np.dtype(image_str.get("dtype", "uint8"))
            ...
            size: tuple[int, int] = tuple(image_str["size"])
            mode = image_str["mode"]
    ...
    for mode, image_args in img2img_image_args_by_mode.items():
        for keys in image_args:
            if mode != args["mode"]:
                ...
                args[keys[0]] = serialize_image(image)  # ty:ignore[invalid-argument-type]
            else:
...
def serialize_script_args(script_args: list):
    ...
    serialized_args = script_args.copy()
    for i, a in enumerate(serialized_args):
        if type(a).__name__ == "UiControlNetUnit":
            serialized_args[i] = serialize_controlnet_args(a)

    return zlib.compress(pickle.dumps(serialized_args))


def deserialize_script_args(script_args: Union[bytes, List], ui_control_net_unit=None):
    ...
    for i, a in enumerate(script_args):
        if isinstance(a, dict) and a.get("is_cnet", False):
            ...
            if ui_control_net_unit is not None:
                u = ui_control_net_unit()
                for k, v in unit.items():
                    if isinstance(getattr(u, k, None), Enum):
                        ...
                        if v not in [e.value for e in enum_obj.__class__]:
                            log.error(
                                f"Invalid enum value {v} for {k} encountered, valid value is {enum_obj.__class__}"
                            )
                            skip_controlnet = True
                            break
                        unit[k] = type(getattr(u, k))(v)
                if not skip_controlnet:  # valid
                    unit = ui_control_net_unit(**unit)
            if not skip_controlnet:  # valid
...
def map_named_script_args_to_list(
    script: scripts.Script, named_args: dict | list
) -> list:
    script_name = script.title().lower()
...
def map_ui_task_args_to_api_task_args(
    ...
    api_task_args: dict[Any, Any | list] = named_args.copy()

    prompt_styles: list = api_task_args.pop("prompt_styles", [])
